...
<%
    ...
    try {
        String protocol = "https"; //request.getProtocol().toLowerCase().substring(0, request.getProtocol().indexOf("/"));
        ...
        obj.setItemName("鍵設信用卡-" + order.getId() + " $" + order.getTotalAmountWithFee() + "元"); //請將非常好書改成你的web site 中文/英文名稱
        obj.setMerchantID("2000132");//必須是2000132
        obj.setMerchantTradeDate(LocalDate.now().toString().replace('-', '/') 
        			+ " " + LocalTime.now().toString().substring(0, 8));//必須是yyyy/MM/dd hh:mm:ss
		String orderIdStr = "KBSHOP" //請改成你的webapp名稱,產生一個allpay要求的訂單編號字串(必須<20個英數字字元,且不得與之前的訂單重複)
